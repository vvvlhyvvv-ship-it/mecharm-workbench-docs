...
## 步骤
1. 点表落地：从 machine.yaml `opcua:` 节创建 Server 节点树（DB_SW_to_PLC / DB_PLC_to_SW 镜像结构，符号名访问，禁偏移直写）；**轴数组按实机 22 轴建（模拟期占位，2026-09-15 G11 口径），段结构含 AxisMask 字段（mask=0 的轴＝保持当前值；契约原"未用轴位填 0"规则废止）**——契约扩容随契约 V1.3 升版，届时只改 machine.yaml 不改码
