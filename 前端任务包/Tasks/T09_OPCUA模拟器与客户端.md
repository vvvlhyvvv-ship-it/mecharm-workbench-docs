...
2. **过渡期布局口径（2026-09-15 @user 审计指出卡内自相矛盾后明确，必读）**：契约 **V1.2 现状仍是 8 轴口径**（§5.2 `Pos = ARRAY[0..7]`、"未使用轴位一律填 0"、§6.1 回读 `EnableMask` 为 `BYTE`）——而本单**禁自行改契约**（见禁止事项）。故本单按以下口径实现：
...
- **禁自行改契约**：契约 V1.2 的 8 轴布局与"未用轴位填 0"**照实现**（过渡期口径见步骤 2），改动一律走 @user 的契约 V1.3 窗口；**不得把 22 轴＋AxisMask 硬塞进 V1.2 布局**
