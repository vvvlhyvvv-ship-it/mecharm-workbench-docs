...
> ⚠️ **交付实况注（2026-09-16 收单登记，覆盖本卡其余 `core/config.py` 字样——含步骤 2）**：实际交付形态为**包** `core/config/`（`__init__.py` 只 re-export＋`__all__`／`schema.py` 字段名常量与 dataclass 定型／`loader.py` 读盘＋收齐问题＋一次性抛错／`validate.py` 各节强校验；实测 30／156／104／296 行）。原因＝原单文件实测 **488 行** > 04 §4.5-① 的 300 行上限，按 **04 §6.3 T03 派单卡**已预授权的结构拆分（**不是** §4.5-⑦，后者的授权范围只限 kinematics／geometry 两包）。`load_machine(path:str)->MachineConfig` 签名与 `from core.config import load_machine` 调用形态**一字不变**；04 §4 矩阵、§2.2 产出列、§5.5-① 期望列与 03 §2 目录树均已同步回填。
