...
## 指挥侧裁决（2026-09-19，遗留 4 项）

> **裁①（导出目录记忆降级＝同意）**：卡面步骤 5 前提「走 core/project.py 现成 API」经指挥侧实测不成立——该件 grep 导出/目录/记忆零命中，proj/v1 严格 schema（asdict/from_dict 对称）且 299/300（L-8 禁改）。`plc_out._last_dir` 会话内记忆＝诚实降级，追认；工程文件级记忆登记 **L-14**（待 proj/v2 迁移，期 3 备忘）。
> **裁②（plc_preview.py 拆件＝追认）**：实测 plc_out 265＋plc_preview 206（均 ≤300），懒导入防回环在位（plc_out.py:175），与 T07 pathctl／T08 checkctl 先例同构，追认（依据＝L-8 机制：预案外拆件报备→指挥方收单追认）。矩阵已补行，**反扩边界＝仅此一件**，波次 3b 内冻结。
> **裁③（白名单落 ui_config.py＝追认＋勘误）**：实测 `plc_declare_top/table/footer` 在 `core/config/ui_config.py` STR_KEYS＋dataclass 双登记（含「声明类不入 EVIDENCE_KEYS、取证档两档同文」口径注），schema.py 只管 machine.yaml——卡面与矩阵「同步 schema.py」系 T12 时代笔误，指挥侧已勘误矩阵；按 L-9 意图落 ui_config.py 正确。
> **裁④①（编程页签横向滚动条＝登记不阻断）**：T14 既有的呈现性偏差（插入动作钮排最小宽 442＞左栏 383），登记 **L-15**，L-4 素材重录前指挥侧裁量（小修或如实接受）；T17 自身输出区压到 374 不加剧，核可。
